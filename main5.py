...
def set_nested_value(d, key_str, value):
    ...
    分解し、辞書 d に対してネストした構造として value を設定する。
    """
    parts = key_str.split(".")
    current = d
...
def sheet_to_df(wb, sheet_name):
    ...
    try:
        ...
        # シートが空の場合は空の DataFrame を返す
        return pd.DataFrame()
    # 残りの行をデータとして DataFrame 化
...
def main():
    ...
    for filename in os.listdir(input_folder):
        ...
        for sheet_name in wb.sheetnames:
            # シート名をキーとし、最上位のネストは「シート名_list」とする
            ...
            # 各行について、gen-tfvars-flagが "True" の場合のみ処理
