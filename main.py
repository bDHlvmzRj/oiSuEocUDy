...
def parse_nested_key(resource_dict, keys, value):
    ...
    if len(keys) == 1:
        ...
        if key.isdigit():
            ...
            if not isinstance(resource_dict, list):
                resource_dict.clear()
                resource_dict.extend([None] * (index + 1))
            while len(resource_dict) <= index:
                resource_dict.append(None)
            resource_dict[index] = value
        else:
            # 辞書として値を設定
            if key not in resource_dict:
                resource_dict[key] = [] if isinstance(value, list) else {}
    ...
    if key.isdigit():
        ...
        if not isinstance(resource_dict, list):
            resource_dict.clear()
            resource_dict.extend([{} for _ in range(index + 1)])
        while len(resource_dict) <= index:
...
def convert_book_to_hcl(file_path):
    """
    Excelファイル全体をHCL形式に変換。シート名を最上位階層に追加して出力。
    ...
    for sheet_name, data in sheets.items():
        ...
        for _, row in data.iterrows():
            ...
            if '[' in keys[-1]:
                # 配列形式の処理
                array_key, index = keys[-1].split('[')
                index = int(index[:-1])
                keys = keys[:-1] + [array_key, str(index)]

            # 再帰的にネストされた構造を構築
            parse_nested_key(resources[resource], keys, value)

        # シート名を最上位階層として保存
        all_resources[f"{sheet_name}_list"] = resources
    ...
    hcl_output = ""
    for sheet_name, resources in all_resources.items():
...
def debug_main_with_book_and_sheets(file_path):
    """
    デバッグ用HCL形式整形対応版メイン関数。最上位階層にシート名を追加。
