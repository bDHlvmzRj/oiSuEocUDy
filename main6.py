...
def main():
    ...
    for filename in os.listdir(input_folder):
        ...
        for sheet_name in wb.sheetnames:
            ...
            # シート内に gen-tfvars-flag が "true" の行が存在しない場合はスキップする
            if "gen-tfvars-flag" not in df.columns or not df["gen-tfvars-flag"].astype(str).str.strip().str.lower().eq("true").any():
                continue

            # sample.csv によりシート名のマッピングがあれば、正しいリソース種別名に修正
            ...
            # 各行を処理：gen-tfvars-flag が "true" の行のみ対象
