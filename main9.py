...
def main():
    ...
    # sample.csv でシート名マッピング読込
    mapping = {}
    csv_path = "sample.csv"
    if os.path.exists(csv_path):
        dfm = pd.read_csv(csv_path)
        for _, r in dfm.iterrows():
            mapping[str(r["resource-name"]).strip()] = str(r["exact-name"]).strip()

    tfvars_data = {}

    # ――― common.xlsx の読み込み ―――
    common_path = os.path.join(input_folder, "common.xlsx")
    if os.path.exists(common_path):
        wb_c = load_workbook(common_path, data_only=True, read_only=True)
        # 最初のシートを共通変数として読み込む
        sheet0 = wb_c.sheetnames[0]
        df_c = sheet_to_df(wb_c, sheet0)
        # 推奨列名: key, value
        if "key" in df_c.columns and "value" in df_c.columns:
            for _, row in df_c.iterrows():
                k = str(row["key"]).strip()
                v = parse_value(row["value"])
                tfvars_data[k] = v
        else:
            print("common.xlsx の列名は 'key','value' の形式で用意してください。")
    ...
    for fn in os.listdir(input_folder):
        ...
        for sheet in wb.sheetnames:
            ...
            # resource-name 毎にグループ化
            for res in df["resource-name"].dropna().unique():
                rname = str(res).strip()
                grp = df[df["resource-name"].astype(str).str.strip() == rname]
                tfvars_data[top].setdefault(rname, {})

                # policy JSON 出力（IAM/S3）
                if grp["arguments"].astype(str).str.strip().eq("policy").any():
                    for _, row in grp.iterrows():
                        ...
                        try:
                            ...
                            continue
                        outdir = iam_folder if "iam" in sheet.lower() else s3_folder
                        with open(os.path.join(outdir, f"{rname}.json"), "w", encoding="utf-8") as jf:
