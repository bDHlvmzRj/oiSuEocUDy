...
def main():
    ...
    # parameterフォルダ内の全Excelファイルを処理
    excel_files = glob.glob(os.path.join(input_folder, "*.xlsx"))
    for excel_path in excel_files:
        # Excelファイルを読み込む（全シート）
        xls = pd.ExcelFile(excel_path)
        for sheet_name in xls.sheet_names:
            # シート名をキーに、最上位のネストは「シート名_list」とする
            top_key = sheet_name + "_list"
            if top_key not in tfvars_data:
                tfvars_data[top_key] = {}
            # シートをDataFrameとして読み込み
            df = pd.read_excel(xls, sheet_name=sheet_name)
            # ※Excelシートのカラムは "resource-name", "arguments", "value", "gen-tfvars-flag" とする
            # 各行で「gen-tfvars-flag」がTrueの場合のみ処理する
