...
def handle_nested_keys(resource_data, keys, value):
    """
    指定されたキーを元にネストされた辞書/配列を構築。
    """
    current = resource_data
    for key in keys[:-1]:  # 最終キー以外を処理
...
def process_excel_file(file_path):
    """Excelファイルを読み込み、リスト形式でtfvarsを構築"""
    tfvars_data = []
    ...
    with pd.ExcelFile(file_path) as xls:
        for sheet_name in xls.sheet_names:
            ...
            for resource, group in filtered_df.groupby("resource"):
                resource_item = {"type": resource, "data": {}}
                for _, row in group.iterrows():
                    keys = row["arguments"].split(".")
                    value = row["value"]
                    if isinstance(value, str) and value.isdigit():
                        value = int(value)  # 数値型に変換
                    handle_nested_keys(resource_item["data"], keys, value)
                tfvars_data.append(resource_item)

    print(f"Processed data for {file_path}: {tfvars_data}")  # デバッグ用出力
    return tfvars_data

def convert_list_to_hcl(tfvars_list):
    """リストをHCL形式に変換"""
    hcl_items = []
    for item in tfvars_list:
        resource_type = item["type"]
        resource_data = item["data"]
        hcl_items.append(f'{resource_type} = {dict_to_hcl(resource_data)}')
    return "\n\n".join(hcl_items)
...
def save_tfvars(data):
    ...
    with open(output_path, "w") as f:
        f.write(convert_list_to_hcl(data))
